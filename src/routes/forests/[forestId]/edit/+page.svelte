<script>
    ...
    async function stuurUpdate(){
        ...
        const request = await fetch(PUBLIC_API_URL + `/forests/api/v1/forests/${forestData.data.id}`, {
            body: data,
            method: "PATCH",
            headers: headers($jwt)
        });
...
<svelte:head>
  <title>{t("editForest", $language)}</title>
</svelte:head>
        ...
        <div class="space-y-6">
          ...
          <div class="bg-card border border-border rounded-xl p-6">
            <form action="javascript://" bind:this={formulier} onsubmit={stuurUpdate} >
              <ForestForm jwt={$jwt} forest={forestData.data} />
            </form>
          </div>
        </div>
      </div>
